...
Game.updateNews = function (datas) {
    ...
    Game.entities.innerHTML.appendChild("<h2>&middot;星期五&middot;今天 </h2>");

    if (data != null) {

        for (var i = 0; i < datas.length; i++) {
            var art = "<article>" +
                "<h3><em></em>" + data.createDate.getHours() + ":" + data.createDate.getMinutes() + "</h3>" +
                ...
                "<a href=\"#\" class=\"share_btn\" data-news-id=\"5817\" data-news-time=\"201712\" title=\"分享币世界数字货币快讯给好朋友\">分享</a>" +
                "</div>" +
                "</article>";
            Console.log(art);
            Game.entities.innerHTML.appendChild(art);
        }
    }
};
